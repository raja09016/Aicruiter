            ...
            <div className="mt-5">
              ...
              <div className={`p-5 mt-10 flex items-center justify-between rounded-md ${(feedback?.Recommendation =="No" || feedback?.Recommendation =="Not Recommended" ) ? 'bg-red-100':'bg-green-100'  } `}>
               ...
               <h2 className={` font-bold ${(feedback?.Recommendation =="No" || feedback?.Recommendation =="Not Recommended") ? 'text-red-700':'text-green-700'  } `}>Recommendation Msg:</h2>
                <p className={`${(feedback?.Recommendation =="No"  || feedback?.Recommendation =="Not Recommended")? 'text-red-500':'text-green-500'  } `}>{feedback?.RecommendationMsg}</p>

               </div>
               <Button className={`  ${(feedback?.Recommendation =="No" || feedback?.Recommendation =="Not Recommended" )? 'bg-red-700':'bg-green-700'  } `}>  Send Msg</Button>
              </div>
            </div>
          </DialogDescription>
        </DialogHeader>
